# Formel für Stadt Bad Kreuznach V6

**Status:** ✅ v6 – Layout basierend auf Screenshot ("Liste für Gruppenleiter*in" oben, "Teilnehmer*innenliste" unten)  
**Kürzel:** KH-Stadt

## Layout-Übersicht

Die Liste hat **2 separate Bereiche**:

| Bereich | Funktion | Formel-Zelle | Datenzeilen | Kapazität | Besonderheit |
|---------|----------|--------------|-------------|-----------|--------------|
| 1 | MA | **B5** | 5-14 | 10 | Geburtsdatum als `dd.MM.yyyy` |
| 2 | TN | **B20** | 20-55+ | 150* | Geburtsjahr als `yyyy` |

*) Auf Seite 1 bis Zeile 55, danach fortlaufend auf Folgeseiten. Formel ist auf 150 Zeilen ausgelegt.
...
| Key | Event-Typ |
|-----|-----------|
| `Stadt Bad Kreuznach_Soziale_Bildung` | Soziale_Bildung |
| `Stadt Bad Kreuznach_Schulung_Ehrenamtlicher_Mitarbeitenden` | Schulung |
| `Stadt Bad Kreuznach_Politische_Jugendbildung` | Politische_Jugendbildung |

---

## Bereich 1: MA – Zelle B5 (max 10 Zeilen)

**Unterschiede zum Kreis:**
- Key: `Stadt Bad Kreuznach`
- Zelle: **B5** (statt B4)
- Kapazität: **10** (statt 12)
- Geb-Datum: **dd.MM.yyyy** (statt Jahr)
...
=LET(
  ...
  ziel_landkreis; "Stadt Bad Kreuznach";
              ...
              LET(
                ...
                IF(NOT(has_final); IF(debug_mode; "⚠️ Keine MA nach Filter"; "");
                  LET(
                    ...
                    HSTACK(col_name; col_plz_ort; col_datum; col_tage; col_leer)
                  )
                )
              )
            )
...
## Bereich 2: TN – Zelle B20 (max 150 Zeilen)

**Unterschiede zum Kreis:**
- Key: `Stadt Bad Kreuznach`
- Zelle: **B20** (statt B29)
- Layout identisch mit Kreis (Geburtsjahr yyyy)
