# Formel für Alzey-Worms V6

**Status:** ✅ v6 – Paging-Layout mit wiederholenden Headern  
**Kürzel:** AZ

## Layout-Übersicht

Da die Header sich auf jeder Seite wiederholen, werden **3 separate Formeln** verwendet:

| Seite | Formel-Zelle | Daten-Bereich | Zeilen der Daten | Start-Index | Max Zeilen |
|-------|--------------|---------------|------------------|-------------|------------|
| 1 | **B4** | B4:F23 | 1-20 | 1 | 20 |
| 2 | **B28** | B28:F46 | 21-39 | 21 | 19 |
| 3 | **B51** | B51:F70 | 40-59 | 40 | 20 |
...
|--------|------|--------|
| A | Lfd. Nr | *separat* |
| B | Vorname Name | `Vorname` & " " & `Name` |
| C | Wohnort | `Ort` (ohne PLZ) |
| D | Geburtsdatum | `dd.mm.yyyy` |
| E | Kennzeichnung | "B" wenn Behinderung |
| F | Unterschrift | leer |
...
| Key | Event-Typ |
|-----|-----------|
| `Landkreis Alzey-Worms_Soziale_Bildung` | Soziale_Bildung |
| `Landkreis Alzey-Worms_Schulung_Ehrenamtlicher_Mitarbeitenden` | Schulung |
| `Landkreis Alzey-Worms_Politische_Jugendbildung` | Politische_Jugendbildung |
...
=LET(
  ...
  ziel_landkreis; "Landkreis Alzey-Worms";
  ...
  IF(rule_check="NOT_FOUND"; IF(debug_mode; "❌ Key nicht gefunden"; "");
        ...
        LET(
          ...
          IF(start_idx > cnt_final; "";
            LET(
              ...
              col_ort; CHOOSECOLS(page_data; 17);
              col_datum; MAP(CHOOSECOLS(page_data; 14); LAMBDA(d; TEXT(d; "dd.mm.yyyy")));
              col_kennz; MAP(CHOOSECOLS(page_data; 4); LAMBDA(b; IF(OR(b="MmB"; b="MmB_BEGLEITPERSON"); "B"; "")));
              col_leer; MAP(SEQUENCE(page_rows); LAMBDA(x; ""));
              
              HSTACK(col_name; col_ort; col_datum; col_kennz; col_leer)
            )
          )
        )
      )
...
## Seite 2 – Zelle B28 (Zeilen 21-39)
...
## Seite 3 – Zelle B51 (Zeilen 40-59)
