# Formel für Landkreis Altenkirchen V6

**Status:** ✅ v6 – 12 Förderumfang-Codes mit REF-Unterstützung  
**Kürzel:** AK

## Output-Spalten

Die Formel wird in **Spalte B** eingefügt und gibt **6 Spalten** aus (B-G).  
**Spalte A (Lfd. Nr)** wird separat gehandhabt.
...
| E | Alter | Index 26 |
| F | Ver-anst.-Tage | Index 6 (Anwesenheit) |
| G | Unterschrift | leer (manuell) |
...
| Key | Event-Typ |
|-----|-----------|
| `Landkreis Altenkirchen_Soziale_Bildung` | Soziale_Bildung |
| `Landkreis Altenkirchen_Schulung_Ehrenamtlicher_Mitarbeitenden` | Schulung |
| `Landkreis Altenkirchen_Politische_Jugendbildung` | Politische_Jugendbildung |

---

## V6 Spalten-Index-Referenz (TN_LISTE)
...
=LET(
  ...
  ziel_landkreis; "Landkreis Altenkirchen";
          ...
          IF(cnt_final = 0; IF(debug_mode; "⚠️ Keine Personen nach Filter"; "");
            IF(cnt_final < min_tn; IF(debug_mode; "⚠️ Min TN (" & min_tn & ") nicht erreicht. Aktuell: " & cnt_final; "");
              HSTACK(
                ...
                MAP(CHOOSECOLS(final_list_sorted; 16); CHOOSECOLS(final_list_sorted; 17); LAMBDA(plz;ort; plz & ", " & ort));
                MAP(CHOOSECOLS(final_list_sorted; 14); LAMBDA(d; TEXT(d; "yyyy")));
                CHOOSECOLS(final_list_sorted; 26);
                CHOOSECOLS(final_list_sorted; 6);
                MAP(SEQUENCE(cnt_final); LAMBDA(x; ""))
              )
            )
          )
        )
...
**V6 (2026-01-20):**
- Initiale V6 für Landkreis Altenkirchen
- 7-Spalten Output: Lfd.Nr, Name, PLZ+Ort, Geburtsjahr, Alter, Tage, Unterschrift
