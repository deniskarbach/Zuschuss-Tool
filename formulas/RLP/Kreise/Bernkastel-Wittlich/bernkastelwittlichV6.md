# Formel für Bernkastel-Wittlich V6

**Status:** ✅ v6 – Mehrseitiges Layout mit festen Headern (Westerwald-Logic)  
**Kürzel:** BKS
...
| Seite | Formel-Zelle | Daten-Bereich | Zeilen | Kapazität |
|-------|--------------|---------------|--------|-----------|
| 1 | **B4** | B4:E28 | 1-25 | 25 |
| 2 | **B33** | B33:E63 | 26-56 | 31 |
| 3 | **B67** | B67:E97 | 57-87 | 31 |
| 4 | **B101** | B101:E131 | 88-118 | 31 |

> **Hinweis:** Die Header-Zeilen (z.B. 29-32) dürfen nicht überschrieben werden.
...
|--------|------|--------|
| A | Lfd. Nr | *separat* |
| B | Vor- u. Zuname | Index 13 + 12 |
| C | PLZ, Wohnort | Index 16 + 17 |
| D | Geburts-Datum | Index 14 (dd.MM.yyyy) |
| E | Alter | Index 26 |
| F | Teiln.-Tage | Index 6 |
| G | Unterschrift | leer |
...
| Key | Event-Typ |
|-----|-----------|
| `Landkreis Bernkastel-Wittlich_Soziale_Bildung` | Soziale_Bildung |
| `Landkreis Bernkastel-Wittlich_Schulung_Ehrenamtlicher_Mitarbeitenden` | Schulung |
| `Landkreis Bernkastel-Wittlich_Politische_Jugendbildung` | Politische_Jugendbildung |

---

## Seite 1 – Zelle B4 (Zeilen 1-25)
...
=LET(
  ...
  start_idx; IF(page=1; 1; page_size_1 + (page-2)*page_size_next + 1);
  end_idx; IF(page=1; page_size_1; page_size_1 + (page-1)*page_size_next);
  
  ziel_landkreis; "Landkreis Bernkastel-Wittlich";
            ...
            IF(AND(page = 1; cnt_final < min_tn); IF(debug_mode; "⚠️ Min TN (" & min_tn & ") nicht erreicht. Aktuell: " & cnt_final; "");
              IF(start_idx > cnt_final; "";
                LET(
                  ...
                  col_alter; CHOOSECOLS(page_data; 26);
                  col_tage; CHOOSECOLS(page_data; 6);
                  col_leer; MAP(SEQUENCE(page_rows); LAMBDA(x; ""));
                  
                  HSTACK(col_name; col_plz_ort; col_datum; col_alter; col_tage; col_leer)
                )
              )
            )
          )
...
## Seite 2 – Zelle B33 (Zeilen 26-56)
...
## Seite 3 – Zelle B67 (Zeilen 57-87)
...
## Seite 4 – Zelle B101 (Zeilen 88-118)
