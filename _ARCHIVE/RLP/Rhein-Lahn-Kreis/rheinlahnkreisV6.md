# Formel für Rhein-Lahn-Kreis V6

**Status:** ✅ v6 – Erweitert auf 12 Förderumfang-Codes mit REF-Unterstützung

## Änderungen V5 → V6

| Feature | V5 | V6 |
|---------|----|----|
| Förderumfang-Codes | 8 | **12** |
| REF (Referent:in) | ❌ | ✅ |
| Altersfilter | Nur TN | TN (MA/REF ausgenommen) |

### Neue Förderumfang-Codes
...
=LET(
  ...
  ziel_landkreis; CACHE_RULES!B28; 
          ...
          IF(cnt_final = 0; IF(debug_mode; "⚠️ Keine Personen nach Filter"; "");
            IF(cnt_final < min_tn; IF(debug_mode; "⚠️ Min TN (" & min_tn & ") nicht erreicht. Aktuell: " & cnt_final; "");
              HSTACK(
                ...
                CHOOSECOLS(final_list_sorted; 27);
...
**V6 (12 Förderumfang-Codes mit REF):**
- 4 neue Codes: `TN_K_REF`, `TN_K_MA_K_REF`, `TN_P_MA_P_REF`, `REF`
- REF-Logik hinzugefügt: `ref_aktiv` (immer pauschal)
- REF-Filter-Block: `all_ref`, `final_ref`
- Altersfilter: MA und REF ausgenommen
- VSTACK erweitert: TN + MA + REF
- Rückwärtskompatibel mit V5 (8 Codes weiterhin gültig)

**V5 (7 Platzhalter-Spalten nach Geschlecht):**
- 7 Platzhalter-Spalten nach S (Geschlecht) eingefügt: T-Z (Index 19-25)
- Bereichsreferenzen angepasst: `B:V` → `B:AC`
- Landkreis-Spalte in Filterreferenzen: `U` → `AB`
- Alle CHOOSECOLS nach den Platzhaltern: +7
  - Alter: 19 → 26
  - Landkreis: 20 → 27
- Kompatibel mit neuer TN_LISTE-Struktur (29 Spalten)
